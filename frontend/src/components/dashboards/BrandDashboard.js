  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Fetch real opportunities data (company's posted opportunities)
        ...
        const transformedOpportunities = opportunitiesData.length > 0 
          ? opportunitiesData.slice(0, 3).map((opp, index) => ({
              ...
              deadline: opp.applicationDeadline 
                ...
                : '1 week',
              match: Math.floor(Math.random() * 10) + 90, // Higher match for brand opportunities
                ...
                {recentActivity.map((activity) => (
                  <div key={activity.id} className="flex items-center gap-4 p-4 bg-white/[0.02] hover:bg-white/[0.04] border border-white/5 rounded-xl transition-all duration-300 hover:border-white/10">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                      activity.type === 'campaign' || activity.type === 'content_published' ? 'bg-yellow-500/10 border border-yellow-500/20' :
                      activity.type === 'collaboration' || activity.type === 'new_connection' ? 'bg-blue-500/10 border border-blue-500/20' :
                      activity.type === 'application' || activity.type === 'profile_view' ? 'bg-green-500/10 border border-green-500/20' :
                      activity.type === 'connection' || activity.type === 'opportunity_applied' ? 'bg-purple-500/10 border border-purple-500/20' :
                      'bg-orange-500/10 border border-orange-500/20'
                    }`}>
                      {(activity.type === 'campaign' || activity.type === 'content_published') ? <Megaphone className="w-4 h-4 text-yellow-500" /> :
                       (activity.type === 'collaboration' || activity.type === 'new_connection') ? <Users className="w-4 h-4 text-blue-500" /> :
                       (activity.type === 'application' || activity.type === 'profile_view') ? <Eye className="w-4 h-4 text-green-500" /> :
                       (activity.type === 'connection' || activity.type === 'opportunity_applied') ? <User className="w-4 h-4 text-purple-500" /> :
